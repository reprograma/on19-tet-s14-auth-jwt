...
const login = (req, res) => {
  tarefas.findOne({ nome: req.body.nome }, function (error, tarefas) {
    if (!tarefas) {
      return res.status(404).send(`O nome ${req.body.nome} não foi localizado`);
    }

    const senha = bcrypt.compareSync(req.body.senha, tarefas.senha);

    if (!senha) {
      return res.status(403).send(`A senha está incorreta. Tente novamente`);
    }
